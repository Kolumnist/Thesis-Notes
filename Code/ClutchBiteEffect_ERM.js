...
const MAX_SLIP_RPM = 3000; // between 300 to 2500
...
torqueNorm = Math.max(clamp01(Math.abs(torque) / MAX_TORQUE),0.2);

function intensity() {	
    
    bitePointDistance = Math.abs(clutch - CLUTCH_POINT_CENTER) / CLUTCH_POINT_WIDTH;
    if (bitePointDistance > 1.0125 || gear == 1) {
        return 0;
    }
...
let amplitude = intensity() * BASE_AMPLITUDE * (0.7 + 0.3*torqueNorm);

return Math.round(frequency) + ";" + Math.round(amplitude*100) + "\n";
